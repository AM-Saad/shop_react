...
const AddToCartBtn: React.FC<Props> = (props) => {
    ...
    const clickHandler = (e: any) => {
        setIsClicked(true)
        props.onClicked(e)
        setTimeout(() => toggle_cart!(true), 3000);
        setTimeout(() => setIsClicked(false), 4000);
    }
    return (
        <button ref={buttonRef} onClick={clickHandler} className={`${classes.addToCart} ${props.className} ${isClicked ? classes['checked-out'] : ''}`} >
            <svg viewBox="0 0 24 24" className={`${classes.cart} h-10 w-10`}>
            ...
            <svg className={`${classes.check} h-10 w-10`} viewBox="0 0 24 24">
