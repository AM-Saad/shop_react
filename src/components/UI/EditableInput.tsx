...
const EditableInput: React.FC<{ label: string, inputType: string; defaultVal: string | number, loading: boolean; onSave: (value: string | number) => void }> = ((props, ref) => {
  ...
  useEffect(() => {
    if (!props.loading) {
      dispatchInputState({ type: ActionKind.USER_INPUT, isEdit: false, value: props.defaultVal })
    }
    ...
    <>
      <div className="text-left border-b-1-g mb-6 pb-2 border-b-2">
        <div className="flex items-center mb-2 mt-3">
          ...
          <EditIcon isEdit={inputState.isEdit} startEdit={() => { dispatchInputState({ type: ActionKind.USER_INPUT, isEdit: true, value: defaultVal }) }} />
        </div>

        <p className={`${inputState.isEdit ? 'hidden' : 'block'} mr-5  font-medium text-lg`}>{defaultVal || '.n.c'}</p>
        <div className={`${inputState.isEdit ? 'block' : 'hidden'}`}>
          <input className={`border-1-b bg-lightgray  w-full  p-large b-r-medium mb-2 m-t-large font-medium p-2`} 
          onChange={(e) => setDefault(e.target.value)}
           id={`update_${props.label}`} type={props.inputType} placeholder={`Add ${props.label}`} value={defaultVal} />
          <div className="flex justify-end gap-2">
            <button type="button" className=" py-2 px-4 text-sm bg-gray-400 rounded hover:opacity-70 text-white" onClick={() => dispatchInputState({ type: ActionKind.USER_INPUT, isEdit: false, value: defaultVal })}>Cancel</button>
